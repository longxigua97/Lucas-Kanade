...
def Jacobian(affine_img):
    img_height, img_width = affine_img.shape[:2]
    Jacobian_image = np.zeros_like(affine_img)
    
    for y in range(img_height):
        for x in range(img_width):
            Jacobian_image[x,y] = affine_img[x,y]
...
if (1):
    ...
    image_w = image_Original[70:170,30:130]

    image_template = affine_transform(image_w, transform_matrix)
    ...
    while(i<1):
        if(i==0):
            affined_image = affine_transform(image_w, affine_matrix)
        else:
            affined_image = affine_transform(image_w, affine_matrix)
        #计算模板图和affine图之间的差异
        ...
        grad_x_img1 = np.zeros([100,100])
        grad_x_img2 = np.zeros([100,100])
        grad_x_img3 = np.zeros([100,100])
        grad_x_img4 = np.zeros([100,100])
        grad_x_img5 = np.zeros([100,100])
        grad_x_img6 = np.zeros([100,100])
        # 遍历输出图像的每个像素
        for x in range(img_height):
            for y in range(img_width):
                # gradient * Jacobian
                    ...
                    #print(Steepest_Descent,Hessian) 
                    grad_x_img1[x,y] = Steepest_Descent[0,0]
                    grad_x_img2[x,y] = Steepest_Descent[0,1]
                    grad_x_img3[x,y] = Steepest_Descent[0,2]
                    grad_x_img4[x,y] = Steepest_Descent[0,3]
                    grad_x_img5[x,y] = Steepest_Descent[0,4]
                    grad_x_img6[x,y] = Steepest_Descent[0,5]
        ...
        plt.subplot(2, 7, 4)
        plt.imshow(grad_x_img1, cmap='gray')
        plt.title('grad_x_img1')
        
        plt.subplot(2, 7, 5)
        plt.imshow(grad_x_img2, cmap='gray')
        plt.title('grad_x_img2')
        
        plt.subplot(2, 7, 6)
        plt.imshow(grad_x_img3, cmap='gray')
        plt.title('grad_x_img3')
        
        plt.subplot(2, 7, 7)
        plt.imshow(grad_x_img4, cmap='gray')
        plt.title('grad_x_img4')
        
        plt.subplot(2, 7, 8)
        plt.imshow(grad_x_img5, cmap='gray')
        plt.title('grad_x_img5')
        
        plt.subplot(2, 7, 9)
        plt.imshow(grad_x_img6, cmap='gray')
        plt.title('grad_x_img6')
        
        plt.subplot(2, 7, 10)
        plt.imshow(Hessian_matrix,cmap='gray')
        plt.title('Hessian_matrix')
        
        plt.subplot(2, 7, 11)
        plt.imshow(np.linalg.inv(Hessian_matrix),cmap='gray')
        plt.title('Inverse Hessian')
    ...
    """
                    ...
                    Descent = Descent + Steepest_Descent.T * difference_image[x,y]
                    #print(Descent.shape)    
        #print(Hessian_matrix)  
        #print(Descent)
        
        Delta_P = np.matmul(Hessian_matrix**(-1),Descent)  
        print(Delta_P)
