...
def steepest_descent(grad_x, grad_y, x, y):
    ...
    #数据格式准备
    grad_fusion[0,0] = grad_x[y,x]
    grad_fusion[0,1] = grad_y[y,x]
...
if (1):
    ...
    image_template = resize_image(image_Original, transform_matrix)
    image_template = image_template[w_x_min:w_x_max,w_y_min:w_y_max]
    image_w = image_Original[w_x_min:w_x_max,w_y_min:w_y_max]
    ...
    while(i<30):
        if(i==0):
            affined_image = resize_image(image_Original[w_x_min:w_x_max,w_y_min:w_y_max], affine_matrix)
        else:
            affined_image = resize_image(image_Original[w_x_min:w_x_max,w_y_min:w_y_max], affine_matrix)
        #计算模板图和affine图之间的差异
